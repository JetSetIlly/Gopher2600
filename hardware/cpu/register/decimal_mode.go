...
func (r *Register) AddDecimal(v interface{}, carry bool) (rcarry bool) {
	if r.size != 8 {
		panic("decimal mode addition only supported for uint8 values with 8 bit registers")
	}
...
func (r *Register) SubtractDecimal(v interface{}, carry bool) (rcarry bool) {
	if r.size != 8 {
		panic("decimal mode subtraction only supported for uint8 values with 8 bit registers")
	}
