...
func (arm *ARM) decodeThumb2Miscellaneous(opcode uint16) decodeFunction {
	...
	if opcode&0xff00 == 0xbf00 {
		if opcode&0xff0f == 0xbf00 {
			return arm.decodeThumb2MemoryHints
		} else {
			return arm.decodeThumb2IfThen
		}
	} else {
		if opcode&0xff00 == 0xbe00 {
			...
			return arm.decodeThumb2ReverseBytes
			...
			return arm.decodeThumb2ChangeProcessorState
			...
			return arm.decodeThumb2CompareAndBranchOnNonZero
		} else if opcode&0xff00 == 0xb200 {
			// sign/zero extend
			return arm.decodeThumb2SignZeroExtend
...
func (arm *ARM) decodeThumb2ReverseBytes(opcode uint16) *DisasmEntry {
	opc := (opcode & 0x00c0) >> 6
	Rn := (opcode & 0x0038) >> 3
	Rd := opcode & 0x0007
...
func (arm *ARM) decodeThumb2MemoryHints(opcode uint16) *DisasmEntry {
...
func (arm *ARM) decodeThumb2IfThen(opcode uint16) *DisasmEntry {
	if arm.decodeOnly {
		return &DisasmEntry{
			Operator: "IT",
		}
...
func (arm *ARM) decodeThumb2CompareAndBranchOnNonZero(opcode uint16) *DisasmEntry {
...
func (arm *ARM) decodeThumb2SignZeroExtend(opcode uint16) *DisasmEntry {
	op := (opcode & 0xc0) >> 6
	Rm := (opcode & 0x38) >> 3
	Rd := opcode & 0x07
