	...
	if arm.function32bitOpcode&0xfe00 == 0xea00 {
		...
		case 0b0010:
			...
			if Rn == rPC {
				...
				case 0b10:
					...
					if signExtend == 0x01 {
						arm.registers[Rd] |= ^uint32(0) << (32 - imm5)
					}
		...
		case 0b1000:
			if Rd == rPC {
				...
				// "4.6.4 ADD (register)"
				arm.fudge_thumb2disassemble32bit = "ADD (register)"
		...
		if opcode&0x0080 == 0x0000 {
			...
			case 0b10:
				...
				// perform sign extension
				if signExtend == 0x01 {
					arm.registers[Rd] |= ^uint32(0) << (32 - shift)
				}
	...
	if arm.function32bitOpcode&0xfa00 == 0xf000 {
		...
		case 0b1000:
			if arm.function32bitOpcode&0x100 == 0x100 {
				...
				result, carry, overflow := AddWithCarry(arm.registers[Rn], imm32, 0)
				arm.registers[Rd] = result
		...
		case 0b1110:
			...
			arm.fudge_thumb2disassemble32bit = "RSB (immediate)"
		...
		if l == 0x1 {
			...
			case 0b11101:
				...
				if regList&0x8000 == 0x8000 {
					arm.registers[rPC] = (arm.read32bit(addr) + 2) & 0xfffffffe
				}
			default:
