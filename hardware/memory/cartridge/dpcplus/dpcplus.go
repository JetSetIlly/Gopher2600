...
func NewDPCplus(instance *instance.Instance, data []byte) (mapper.CartMapper, error) {
	...
	cart.arm = arm.NewARM(cart.version.mamcr, cart.version.mmap, instance.Prefs.ARM, cart.state.static, cart)
...
func (cart *dpcPlus) PlumbFromDifferentEmulation() {
	cart.arm = arm.NewARM(arm.MAMfull, cart.version.mmap, cart.instance.Prefs.ARM, cart.state.static, cart)
}
