...
func (cart *Cartridge) fingerprint(cartload cartridgeloader.Loader) error {
	...
	if ok, version := fingerprintCDF(cartload.Data); ok {
		cart.mapper, err = cdf.NewCDF(cart.prefs, cartload.Filename, version, cartload.Data)
		return err
	}

	if fingerprintDPCplus(cartload.Data) {
		cart.mapper, err = dpcplus.NewDPCplus(cart.prefs, cartload.Filename, cartload.Data)
		return err
	}
