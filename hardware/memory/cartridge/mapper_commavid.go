...
func newCommaVid(env *environment.Environment) (mapper.CartMapper, error) {
	...
	if len(data) < 2048 {
		...
		logger.Log(env, "CV", "placing undersized commavid data at end of cartridge memory")
	} else if len(data) == 2048 {
		copy(cart.bankData[2048:], data[:2048])
		logger.Log(env, "CV", "placing 2k commavid data at end of cartridge memory")
	} else {
