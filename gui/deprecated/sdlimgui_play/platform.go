...
func newPlatform(img *SdlImguiPlay) (*platform, error) {
	...
	plt.window, err = sdl.CreateWindow(windowTitle,
		...
		800, 600,
		sdl.WINDOW_OPENGL|sdl.WINDOW_ALLOW_HIGHDPI|sdl.WINDOW_HIDDEN)
...
// fitDisplaySize resizes the window to the size of the screen
func (plt *platform) fitDisplaySize() {
	plt.window.SetSize(int32(plt.img.screen.scaledWidth()), int32(plt.img.screen.scaledHeight()))
}
...
func (plt *platform) newFrame() {
	if plt.showOnNextStable && plt.img.tv.IsStable() {
		plt.window.Show()
		plt.showOnNextStable = false
	}
	...
	if plt.img.scr.isCaptured && !plt.img.scr.isHovered {
		state = 0
	}
