...
// InitialiseFromCartridge initialises a symbols table from the symbols file for the specified
// cartridge. Even in the event of an error the Symbols table will still be usable and will contain
// the standard 2600 symbols.
//
// Currently, only symbols files generated by DASM are supported.
func (sym *Symbols) InitialiseFromCartridge(cart *cartridge.Cartridge) error {
	// AfterLabelChange() and AfterSymbolChange() both acquire the critical lock so we defer them
	// before locking/unlocking in this function
	...
	sym.label = make([]*table, cart.NumBanks())
