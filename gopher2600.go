...
func emulate(mode string, sync *mainSync, args []string) error {
	...
	if emulationMode == govern.ModePlay {
		...
		flgs.BoolVar(&opts.PlaybackCheckROM, "playbackCheckROM", true, "check ROM hash on playback. playback files only")
		flgs.BoolVar(&opts.PlaybackCheckHashes, "playbackCheckHashes", true, "check video hashes at each playback step. playback files only")
		flgs.BoolVar(&opts.PlaybackPowerOff, "playbackPowerOff", true, "obey power off instructions. playback files only")
...
func disasm(mode string, args []string) error {
	...
	case 1:
		...
		cartload, err := cartridgeloader.NewLoaderFromFilename(args[0], mapping, "AUTO", nil, nil)
		if err != nil {
			return err
		}
...
func perform(mode string, args []string) error {
	...
	case 0:
		return fmt.Errorf("2600 cartridge required")
	case 1:
		cartload, err := cartridgeloader.NewLoaderFromFilename(args[0], mapping, bank, nil, nil)
		if err != nil {
			return err
		}
		defer cartload.Close()
