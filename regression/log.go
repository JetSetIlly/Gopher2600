...
func (reg *LogRegression) regress(newRegression bool, messages io.Writer, tag string) error {
	...
	cartload, err := cartridgeloader.NewLoaderFromFilename(reg.Cartridge, reg.Mapping, "AUTO", nil, tv)
	if err != nil {
		return err
	}
	...
	err = vcs.RunForFrameCount(reg.NumFrames, func() (govern.State, error) {
		...
		case <-tck.C:
			...
			fmt.Fprintf(messages, "\r%s [%d/%d (%.1f%%)]", tag, frame, reg.NumFrames, 100*(float64(frame)/float64(reg.NumFrames)))
		default:
		}
